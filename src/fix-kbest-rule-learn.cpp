...
int is_tagged_with(char *tag,char *tagseq)
...
char *first_tag_nospace(char *tagstr)
...
int main(int             argc, char           *argv[])
{
	...
	int             count,tempcount1,tempcount2;
        size_t          ccount;
	char            old[10], *neww, when[300], tag[100], lft[100], rght[100],
...
/* Bart 20030415	while (gets(line) != NULL) {*/
	while ((myline = mygets()) != NULL) {
	  if (strlen(myline) > 1){
...
/*	  while(fgets(line,linelen,allowedmovefile) != NULL) {*/
	  ...
	  if (strlen(line) > 1) {
		...
		if (! Registry_get(WORDS,curwd) ||
		  ...
		  if (strcmp(when,"ALWAYS") == 0) {
			...
			strcmp(rght, first_tag_nospace((char *)Darray_get(tag_corpus_array, count + 1))) == 0){
		      strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
		      Darray_set(tag_corpus_array, count, curtagcpy); }
		  ...
		  if (count < corpus_size) {
			...
			0 &&
			strcmp(next1, (char *)Darray_get(word_corpus_array, count+1)) ==
			0){
		      strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
		  ...
		  if (count < corpus_size) {
		    if (strcmp(word, (char *)Darray_get(word_corpus_array, count)) ==
			0 &&
			strcmp(next1, first_tag_nospace((char *)Darray_get(tag_corpus_array, count+1))) ==
			0){
		      strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
		  ...
		  if (count < corpus_size) {
		    if (strcmp(word, (char *)Darray_get(word_corpus_array, count)) ==0) {
		      strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
		      Darray_set(tag_corpus_array, count, curtagcpy);
		  ...
		  if (count < corpus_size - 1) {
		    if (strcmp(tag, first_tag_nospace((char *)Darray_get(tag_corpus_array, count + 2))) == 0){
		      strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
		      Darray_set(tag_corpus_array, count, curtagcpy);}
		  ...
		  if (count < corpus_size - 1) {
		    if (strcmp(word, (char *)Darray_get(word_corpus_array, count + 2)) == 0){
		      strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
		      Darray_set(tag_corpus_array, count, curtagcpy);}
		  ...
		  if (count < corpus_size - 1) {
		    if
		      (strcmp(next1, first_tag_nospace((char *)Darray_get(tag_corpus_array, count + 1))) == 0 &&
		       strcmp(next2, first_tag_nospace((char *)Darray_get(tag_corpus_array, count + 2))) == 0){
			strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
			Darray_set(tag_corpus_array, count, curtagcpy);}
		  ...
		  if (count < corpus_size) {
		    ...
		    if
		      (strcmp(tag, first_tag_nospace((char *)Darray_get(tag_corpus_array, count + 1))) == 0 ||
		       strcmp(tag, first_tag_nospace((char *)Darray_get(tag_corpus_array, tempcount1))) == 0 ||
		       strcmp(tag, first_tag_nospace((char *)Darray_get(tag_corpus_array, tempcount2))) == 0){
			strcat(curtag,"_");
			strcat(curtag,neww);
			curtagcpy = mystrdup(curtag);
			Darray_set(tag_corpus_array, count, curtagcpy);}
		  ...
		  if (count < corpus_size) {
		    ...
		    if
		      (strcmp(word, (char *)Darray_get(word_corpus_array, count + 1)) == 0 ||
		       strcmp(word, (char *)Darray_get(word_corpus_array, tempcount1)) == 0 ||
		       strcmp(word, (char *)Darray_get(word_corpus_array, tempcount2)) == 0){
			strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
			Darray_set(tag_corpus_array, count, curtagcpy);}
		  ...
		  if (count > 0) {
		    if (strcmp(word, (char *)Darray_get(word_corpus_array, count)) ==
			0 &&
			strcmp(prev1, (char *)Darray_get(word_corpus_array, count-1)) ==
			0){
		      strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
		  ...
		  if (count > 0) {
		    if (strcmp(word, (char *)Darray_get(word_corpus_array, count)) ==
			0 &&
			strcmp(prev1, first_tag_nospace((char *)Darray_get(tag_corpus_array, count-1))) ==
			0){
		      strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
		  ...
		  if (count > 0) {
		    if (count > 1) 
		      ...
		      tempcount1 = count-1;
		    if (strcmp(tag, first_tag_nospace((char *)Darray_get(tag_corpus_array, count - 1))) == 0 ||
			strcmp(tag, (char *)Darray_get(tag_corpus_array, tempcount1)) == 0){
		      strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
		      Darray_set(tag_corpus_array, count, curtagcpy);}
		  ...
		  if (count > 1) {
		    if (strcmp(prev1, first_tag_nospace((char *)Darray_get(tag_corpus_array, count - 2))) == 0 &&
			strcmp(prev2, first_tag_nospace((char *)Darray_get(tag_corpus_array, count - 1))) == 0){
		      strcat(curtag,"_");
		      strcat(curtag,neww);
		      curtagcpy = mystrdup(curtag);
		      Darray_set(tag_corpus_array, count, curtagcpy);}
	...
	for (count = 0; count <= corpus_size; ++count) {
	  strcpy(tempstr, (char *)Darray_get(tag_corpus_array, count));
	  if (strcmp(tempstr, "STAART") == 0 &&
	      strcmp((char *)Darray_get(tag_corpus_array, count + 1), "STAART") == 0 &&
	      count !=0 )
	    printf("\n");
	  else if (strcmp(tempstr, "STAART") != 0)
	    printf("%s/%s ", (char *)Darray_get(word_corpus_array, count), tempstr);
	}
	printf("\n");
