...
NORET Darray_hint(Darray dyn_ary, unsigned addl_hint, unsigned addh_hint)
    ...
    grow_amount = addh_hint - (temp->storage_length - temp->storage_offset - temp->length);
    if (grow_amount > 0) 
...
NORET Darray_addh(Darray_rep *dyn_ary, VOIDP object)
    {
    Darray_rep *temp = dlower(dyn_ary);
    unsigned /*Bart 20001218*/ int add_posn = temp->storage_offset + temp->length;
...
NORET Darray_addl(Darray dyn_ary, VOIDP obj)
    {
    Darray_rep *temp = dlower(dyn_ary);
...
Bool Darray_valid_index(Darray dyn_ary, unsigned index)
    {
    return Bool_create(index < Darray_len(dyn_ary));
    }

NORET Darray_set(Darray dyn_ary, unsigned index, VOIDP obj)
    {
    Darray_rep *temp = dlower(dyn_ary);
...
VOIDP Darray_get(Darray dyn_ary, unsigned index)
    {
    Darray_rep *temp = dlower(dyn_ary);
...
NORET Darray_values(Darray dyn_ary, VOIDP *c_ary)
    {
    Darray_rep *temp = dlower(dyn_ary);
    VOIDP *p, *q;
...
/* Both of thes function has to be rewritten to use the internal darray data */
...
/*--------------------------------------------------*/
Darray Darray_insert(Darray dar, int index, VOIDP data)
    {
