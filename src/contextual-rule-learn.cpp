...
void implement_change(
    ...
    int corpuslen,
    Registry *wordreg,
    Registry *seentaggingreg
);
char staart[] = "STAART";
char **correct_tag_corpus, **word_corpus, **guess_tag_corpus;
int correct_tag_corpus_index, word_corpus_index;
unsigned int guess_tag_corpus_index;
int localbest;
...
/***********************************************************************/
void increment_array_create(Registry *thehash, char  *thestr)
    {
    int *numforhash;
...
/***********************************************************************/
/********************************************************************/
void decrement_array(Registry *thehash, char  *thestr)
    {
    int *numforhash;
...
/***********************************************************************/
void decrement_array_create(Registry *thehash, char  *thestr)
    {
    int *numforhash;
...
/***********************************************************************/
void check_counts(Registry *goodhash, char *label)
    {
...
void init_hash(Registry *thehash, int numhint)
    {
...
int main(int             argc,
    ...
    char tempfile1[512], tempfile2[512];
    ...
    if (argc < 5)
        {
        printf("Need 4 arguments, got %d\n", argc - 1);
        exit(-1);
        }
    allowedmovefile = fopen(argv[4], "r");
    if (allowedmovefile == NULL)
        {
        fprintf(stderr, "%s %s\n", "Cannot open allowedmovefile argv[4] for reading", allowedmovefile);
        exit(-1);
        }
    ...
    if (correct_file == NULL)
        {
        fprintf(stderr, "%s %s\n", "Cannot open correct_file argv[1] for reading.", correct_file);
        exit(-1);
        }
    ...
    while (fgets(line, sizeof(line), correct_file) != NULL) {
        if (not_just_blank(line)) {
            ...
            while (*split_ptr != NULL && **split_ptr != '\r') {
    ...
    if (guess_file == NULL)
        {
        fprintf(stderr, "%s %s\n", "Cannot open guess_file argv[2] for reading.", guess_file);
        exit(-1);
        }
    ...
    if (words_in_good != words_in_bad) {
        printf("%s and %s do not have the same number of words (words_in_good %d words_in_bad %d)\n", argv[1], argv[2], words_in_good, words_in_bad);
        /*    exit(0);*/
        }
    else
        {
        ...
        while (CONTINUE > THRESHOLD) { /* this is the rule learning loop.
            ...
            if (error_list == NULL)
                {
                fprintf(stderr, "%s %s\n", "Cannot open error_list for appending.", error_list);
                exit(-1);
                }
            ...
            if (error_list == NULL)
                {
                fprintf(stderr, "%s %s\n", "Cannot open error_list for reading.", error_list);
                exit(-1);
                }
            ...
            if (correct_out == NULL)
                {
                fprintf(stderr, "%s %s\n", "Cannot open correct_out argv[3] for appending", correct_out);
                exit(-1);
                }
...
void implement_change(
    char *thearg,
    char **thetagcorpus, char **thewordcorpus,
    int corpuslen,
    Registry *wordreg, Registry *seentaggingreg
)
    {
