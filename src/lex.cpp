...
int numspaces(char* buf)
    ...
    size_t count;
    /*
    Goal: return number of separators
...
char** perl_split(char* buf)
    ...
    INCREMENT
        return_buf[cntr++] = (char*)strtok(temp2, " \t\v\f");
    while((temp = (char*)strtok(NULL, " \t\v\f")) != NULL)
        {
        /*if(temp != NULL)*/
            {
...
/*    while(*buf == ' ' || *buf == '\t' || *buf == '\v')
    ...
    INCREMENT
        return_buf[cntr++] = (char*)mystrdup((char*)strtok(buf, " \t\v\f"));
    while((temp = (char*)strtok(NULL, " \t\v\f")) != NULL)
        {
