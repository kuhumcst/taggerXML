...
/* Creates and returns an empty directory */
...
/* Finds a named object in a directory and returnd the original key */
 ...
 /* In this case, a pointer to the name should be stored, then the */
...
NORET Registry_traverse(Registry dir, Registry_ActionProc action, VOIDP priv_ptr)
    {
    RegistryRecord *p;
    unsigned int i;
...
static NORET add_to_darrays(VOIDP key, VOIDP value, VOIDP priv_ptr)
    {
...
*/
unsigned int Registry_strhash(CONSTVOIDP strp, unsigned int htsize)
    {
    if(htsize)
        {
        char *cp = (char *)strp;
        ...
        return ((unsigned int)hash_temp) % htsize;
        }
    else
        return 0;
    }

unsigned int Registry_strcasehash(CONSTVOIDP strp, unsigned int htsize)
    {
    if(htsize)
        {
        char *cp = (char *)strp;
...
int Registry_ptrcmp(CONSTVOIDP ptr1, CONSTVOIDP ptr2)
    {
    if (ptr1==ptr2)
        return 0;
    else
...
static void regfree(void * name, void * obj, void * priv_ptr)
    {
    if(name > (void *) 1)
        {
        free(name);
...
static void regfreeonlynames(void * name, void * obj, void * priv_ptr)
    {
    if(name > (void *) 1)
        {
        free(name);
...
void freeRegistry(Registry dir)
    {
    if(dir)
        {
        Registry_traverse(dir, regfree, NULL);
        Registry_destroy(dir);
        }
    }


void freeRegistryOnlyNames(Registry dir)
    {
    if(dir)
        {
        Registry_traverse(dir, regfreeonlynames, NULL);
        Registry_destroy(dir);
        }
    }
